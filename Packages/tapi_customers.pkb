create or replace PACKAGE BODY tapi_customers IS

   /**
   * TAPI_CUSTOMERS
   * Generated by: tapiGen2 - DO NOT MODIFY!
   * Website: github.com/osalvador/tapiGen2
   * Created On: 23-JUL-2016 16:51
   ...
   */
   ...
   CURSOR customers_cur (
                       p_user_id IN customers.user_id%TYPE
                       )
   IS
      SELECT
            user_id,
            state_id,
            tapi_customers.hash(user_id),
            ROWID
      FROM customers
      WHERE
           user_id = customers_cur.p_user_id
      FOR UPDATE;

    --By Rowid
    CURSOR customers_rowid_cur (p_rowid IN VARCHAR2)
    IS
      SELECT
             user_id,
             state_id,
   ...
   FUNCTION rt (
      ...
      RETURN customers_rt RESULT_CACHE
   IS
      l_scope logger_logs.scope%TYPE := gc_scope_prefix || 'rt';
      l_params logger.tab_param;
      l_customers_rec customers_rt;
   BEGIN
      logger.append_param(l_params, 'p_user_id', p_user_id);
      logger.LOG('START', l_scope, NULL, l_params);
   ...
   END rt_for_update;
    ...
    BEGIN
      ...
      logger.LOG('Populating record type from DB', l_scope);
       ...
       LOOP
              l_customers_rec.user_id := c1.user_id;
              l_customers_rec.state_id := c1.state_id;
              l_customers_rec.hash := tapi_customers.hash( c1.user_id);
              l_customers_rec.row_id := c1.ROWID;
              PIPE ROW (l_customers_rec);
       END LOOP;
    ...
    PROCEDURE ins (p_customers_rec IN OUT customers_rt)
    IS
        l_scope logger_logs.scope%TYPE := gc_scope_prefix || 'ins';
        l_params logger.tab_param;
        l_rowtype     customers%ROWTYPE;

    BEGIN
        logger.append_param(l_params, 'p_customers_rec.user_id', ins.p_customers_rec.user_id);
        logger.append_param(l_params, 'p_customers_rec.state_id', ins.p_customers_rec.state_id);
        logger.LOG('START', l_scope, NULL, l_params);
        logger.LOG('Inserting data', l_scope);


        l_rowtype.user_id := ins.p_customers_rec.user_id;
        l_rowtype.state_id := ins.p_customers_rec.state_id;
    ...
    PROCEDURE upd (
                  p_customers_rec         IN customers_rt,
    ...
    IS
      ...
      l_params logger.tab_param;
    BEGIN
       logger.append_param(l_params, 'p_customers_rec.user_id', upd.p_customers_rec.user_id);
       logger.append_param(l_params, 'p_customers_rec.state_id', upd.p_customers_rec.state_id);
       logger.LOG('START', l_scope, NULL, l_params);
       ...
       THEN
          UPDATE   customers
             SET user_id = NVL(p_customers_rec.user_id,user_id),
                state_id = NVL(p_customers_rec.state_id,state_id)
           WHERE
                user_id = upd.p_customers_rec.user_id
                ;
       ELSE
          UPDATE   customers
             SET user_id = p_customers_rec.user_id,
                state_id = p_customers_rec.state_id
           WHERE
                user_id = upd.p_customers_rec.user_id
                ;
       END IF;
    ...
    IS
      ...
      l_params logger.tab_param;
    BEGIN
       logger.append_param(l_params, 'p_customers_rec.user_id', upd_rowid.p_customers_rec.user_id);
       logger.append_param(l_params, 'p_customers_rec.state_id', upd_rowid.p_customers_rec.state_id);
       logger.LOG('START', l_scope, NULL, l_params);
       logger.LOG('Updating table', l_scope);
       ...
       THEN
          UPDATE   customers
             SET user_id = NVL(p_customers_rec.user_id,user_id),
                state_id = NVL(p_customers_rec.state_id,state_id)
           WHERE  ROWID = p_customers_rec.row_id;
       ELSE
          UPDATE   customers
             SET user_id = p_customers_rec.user_id,
                state_id = p_customers_rec.state_id
   ...
   IS
      ...
      l_customers_rec customers_rt;
   BEGIN
       logger.append_param(l_params, 'p_customers_rec.user_id', web_upd.p_customers_rec.user_id);
       logger.append_param(l_params, 'p_customers_rec.state_id', web_upd.p_customers_rec.state_id);
       logger.LOG('START', l_scope, NULL, l_params);
       logger.LOG('Updating table', l_scope);

      OPEN customers_cur(
                             web_upd.p_customers_rec.user_id
                        );

      FETCH customers_cur INTO l_customers_rec;

      IF customers_cur%NOTFOUND THEN
         CLOSE customers_cur;
         RAISE e_row_missing;
      ELSE
         IF p_customers_rec.hash != l_customers_rec.hash THEN
            CLOSE customers_cur;
         ...
         ELSE
            ...
            THEN
                ...
                UPDATE   customers
                   SET user_id = NVL(p_customers_rec.user_id,user_id),
                       state_id = NVL(p_customers_rec.state_id,state_id)
               WHERE CURRENT OF customers_cur;
            ELSE
                UPDATE   customers
                   SET user_id = p_customers_rec.user_id,
                       state_id = p_customers_rec.state_id
   ...
   IS
      ...
      l_customers_rec customers_rt;
   BEGIN
       logger.append_param(l_params, 'p_customers_rec.user_id', web_upd_rowid.p_customers_rec.user_id);
       logger.append_param(l_params, 'p_customers_rec.state_id', web_upd_rowid.p_customers_rec.state_id);
       logger.LOG('START', l_scope, NULL, l_params);
       logger.LOG('Updating table', l_scope);

      OPEN customers_rowid_cur(web_upd_rowid.p_customers_rec.row_id);

      FETCH customers_rowid_cur INTO l_customers_rec;

      IF customers_rowid_cur%NOTFOUND THEN
         CLOSE customers_rowid_cur;
         RAISE e_row_missing;
      ELSE
         IF web_upd_rowid.p_customers_rec.hash != l_customers_rec.hash THEN
            CLOSE customers_rowid_cur;
            RAISE e_ol_check_failed;
         ELSE
            ...
            THEN
                UPDATE   customers
                   SET user_id = NVL(p_customers_rec.user_id,user_id),
                       state_id = NVL(p_customers_rec.state_id,state_id)
               WHERE CURRENT OF customers_rowid_cur;
            ELSE
                UPDATE   customers
                   SET user_id = p_customers_rec.user_id,
                       state_id = p_customers_rec.state_id
   ...
   BEGIN
      ...
      IF customers_cur%NOTFOUND THEN
         ...
         RAISE e_row_missing;
      ELSE
         IF web_del.p_hash != l_customers_rec.hash THEN
            CLOSE customers_cur;
            RAISE e_ol_check_failed;
         ELSE
            DELETE FROM customers
   ...
   BEGIN
      ...
      OPEN customers_rowid_cur(web_del_rowid.p_rowid);
      ...
      IF customers_rowid_cur%NOTFOUND THEN
         ...
         RAISE e_row_missing;
      ELSE
         IF web_del_rowid.p_hash != l_customers_rec.hash THEN
            CLOSE customers_rowid_cur;
            RAISE e_ol_check_failed;
         ELSE
            DELETE FROM customers
